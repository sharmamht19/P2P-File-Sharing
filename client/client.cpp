...
//**************************************************|  Piece selection Algorithems  |*************************************************

vector<int> randomPieceSelection(vector<string>& seeders, int NoOfSeedrs, int NoOfChunks, int avail_C_atSeeder[][NoOfChunks + 1]) {
    vector<int> getChunksFor(NoOfChunks + 1, 0);
    int s = 1;

    for (int i = 1; i <= NoOfChunks; i++) {
        vector<int> availableSeeders;
        for (int j = 1; j <= NoOfSeedrs; j++) {
            if (avail_C_atSeeder[j][i] == 1) {
                availableSeeders.push_back(j);
            }
        }

        if (!availableSeeders.empty()) {
            int randomIndex = rand() % availableSeeders.size();
            getChunksFor[i] = availableSeeders[randomIndex];
        }
    }
...
vector<int> rarestPieceSelection(vector<string>& seeders, int NoOfSeedrs, int NoOfChunks, int avail_C_atSeeder[][NoOfChunks + 1]) {
    // Calculate the frequency of each chunk across all seeders
    vector<int> chunkFrequency(NoOfChunks + 1, 0);
    for (int i = 1; i <= NoOfSeedrs; i++) {
        for (int j = 1; j <= NoOfChunks; j++) {
            chunkFrequency[j] += avail_C_atSeeder[i][j];
        }
    }

    // Generate an array that indexes chunks by rarity
    vector<int> rarestOrder(NoOfChunks + 1);
    for (int i = 1; i <= NoOfChunks; i++) {
        rarestOrder[i] = i;
    }
    sort(rarestOrder.begin() + 1, rarestOrder.end(), [&](int a, int b) {
        return chunkFrequency[a] < chunkFrequency[b];
    });

    vector<int> getChunksFor(NoOfChunks + 1, 0);
    for (int i = 1; i <= NoOfChunks; i++) {
        int rarestChunk = rarestOrder[i];
        for (int j = 1; j <= NoOfSeedrs; j++) {
            if (avail_C_atSeeder[j][rarestChunk] == 1) {
                getChunksFor[rarestChunk] = j;
                break;
            }
        }
    }
...
void receiveFile(int new_socket, long long filesize, string fname) {
    cout << endl
         << oprStart << "      Receiveing File........" << printEnd << endl;
    ...
    if (filesize == -100)
        cout << errorStart << "      File Download failed..  " << printEnd << endl;
    else
...
void peer2peer(string fsize, string file_path, string ip, string port, string DestOfFile, string chunks, int seedNo, int FD) {
    ...
    for (int i = 0; i < chunks_list.size() - 1; i++) {
        int PeerSockID = socket_;
        int p_ChunkNo = stoi(chunks_list[i]);
        string filename = DestOfFile;
...
void* start_Server(void* c_PORT) {
    ...
    while (true) {
        ...
        cout << endl
             << oprStart << "          connected from PORT:   " << serverAddress.sin_port << endl;
        ...
        cout << endl
             << oprStart << "          conn. requested for file : " << fname << printEnd << endl;
        ...
        for (int i = 0; i < chunks_list.size() - 1; i++) {
            ...
            int p_NewSOCKID = Newsocket;
            int ChunkNo = stoi(chunks_list[i]);
...
int main(int argc, char* argv[]) {
    ...
    cout << endl
         << "                           "
         << "\033[1;30;43m"
         << " | <client> IP : " << c_IP << " PORT : " << c_PORT << " | " << printEnd << endl;
        ...
        if (command == "create_user" || command == "login" || command == "quit" || Session == 1) {
            ...
            // On command create_user..
                ...
                if (op == "vailid")
                    ...
                    cout << errorStart << "  User already exists.." << printEnd << endl;
                ...
                if (op == "vailid") {
                    ...
                    cout << errorStart << "  You are already LoggedIn somewhere.." << printEnd << endl;
                else
                    cout << errorStart << "  Invailid credentials" << printEnd << endl;
                ...
                if (op == "vailid")
                    ...
                    cout << errorStart << "  Group already exists.." << printEnd << endl;
                ...
                if (op == "invailid")
                    ...
                    string addnl = "";
                    // vector<int> getChunksFor = rarestPieceSelection(seeders, NoOfSeedrs, NoOfChunks, avail_C_atSeeder);
                    // vector<int> getChunksFor = randomPieceSelection(seeders, NoOfSeedrs, NoOfChunks, avail_C_atSeeder);
                    ...
                    if (stat == 1) {
                        ...
                        cout << errorStart << "      File Download failed! please retry after sometime,," << printEnd << endl;
                    for (int k = 1; k <= NoOfSeedrs; k++)
                        seederStatus[k] = 0;
                ...
                cout << errorStart << "  invailid Input!" << printEnd << endl;
        } else
            cout << errorStart << "  Please first login!" << printEnd << endl;
    }
    return 0;
}